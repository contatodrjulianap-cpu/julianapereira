...
const Q = z.object({
  ...
  variant: z.enum(["resina", "porcelana"]).optional(),
});

// GET /api/quiz/wa-link?archetype=PRONTA&phone=5511...&fn=Mariana&variant=resina
// Retorna a URL pronta pro CTA do resultado do quiz.
// - CETICA → Instagram (não passa por WPP)
// - PRONTA/ESPERANCOSA → wa.me/<numero da atendente atribuída>?text=<msg arquétipo + variant>
export async function GET(req: NextRequest) {
  ...
  const msg = whatsappMessageFor(
    parsed.data.archetype as Archetype,
    parsed.data.fn,
    parsed.data.variant as Variant | undefined,
  );
